...
__global__
void activationKernelWithCopy(ConnectionActivation layer, LayerActivation activation,
  ...
  if (row < layer.batchSize && col < layer.activation.cols - 1) {
    ...
    float in = *aElem;
    float av = activationValue(in, activation);
    float dv = activationDerivative(in, av, activation);

    *aElem = av;
    *dElem = dv;

    *Elem(copyLayer.activation, row, col) = av;
    *Elem(copyLayer.derivative, row, col) = dv;
  }
}
...
void ActivationKernel::Apply(const ConnectionActivation &layer, const LayerActivation &activation,
  ...
  assert(layer.activation.rows == layer.derivative.rows);
  assert(layer.activation.cols == layer.derivative.cols);
  assert(layer.activation.rows == copyLayer.derivative.rows);
  assert(layer.activation.cols == copyLayer.derivative.cols);
  assert(layer.batchSize <= layer.activation.rows);
  ...
  activationKernelWithCopy<<<dim3(bpgX, bpgY, 1), dim3(TPB_X, TPB_Y, 1), 0, stream>>>(
      layer, activation, copyLayer);
}
